...
from app.models.database import _use_local_sqlite, DB_FILE, get_supabase
...
async def list_tables(current_user: dict = Depends(get_current_user)) -> list[dict]:
    ...
    """
    db_mode = "SQLite (Local)" if _use_local_sqlite else "Supabase (Cloud)"
...
async def get_table_data(
    ...
    if _use_local_sqlite:
        try:
            ...
            c = conn.cursor()
            # Order by id descending if id column exists, else by rowid
        ...
        try:
            ...
            logger.error("Failed to fetch Supabase table %s: %s", table_name, e)
            # Try SQLite fallback
            try:
                conn = sqlite3.connect(DB_FILE)
                conn.row_factory = sqlite3.Row
                ...
                raise HTTPException(status_code=500, detail=f"Supabase failed and SQLite fallback error: {e}")
        ...
        try:
            ...
            if not resp.data:
                ...
                try:
                    ...
                    conn.close()
                    if changes > 0:
                        return {"status": "deleted", "id": row_id, "table": table_name, "fallback": "SQLite"}
                except Exception:
                    pass
                raise HTTPException(status_code=404, detail="Row not found in Supabase")
            return {"status": "deleted", "id": row_id, "table": table_name}
        except Exception as e:
            logger.error("Failed to delete Supabase row in %s: %s", table_name, e)
            # SQLite fallback
            try:
                conn = sqlite3.connect(DB_FILE)
                c = conn.cursor()
