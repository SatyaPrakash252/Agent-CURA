...
export default function ClinicalAgentOrb({ data, active, elapsed, language }: ClinicalAgentOrbProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const pulseFactor = active ? 1.05 + avg * 1.5 : 1.0;
      const baseRadius = Math.min(w, h) * 0.28 * pulseFactor;

      // Draw glowing background aura
      const radialGlow = ctx.createRadialGradient(cx, cy, baseRadius * 0.3, cx, cy, baseRadius * 1.6);
      radialGlow.addColorStop(0, active ? 'rgba(99, 102, 241, 0.25)' : 'rgba(99, 102, 241, 0.04)');
      ...
      if (active) {
        ...
        for (let r = 0; r < rings; r++) {
          ...
          ctx.lineWidth = 1.2 + (1 - ratio) * 2;
          ctx.beginPath();
          ctx.arc(cx, cy, baseRadius + ratio * 60, 0, Math.PI * 2);
          ctx.stroke();
        }
        ...
        const maxLen = 28 * (0.8 + avg * 2.2);
        for (let i = 0; i < barCount; i++) {
          const angle = (i / barCount) * Math.PI * 2 + (Date.now() * 0.0001);
          const rawVal = data[i % data.length] || 0.05;
          const len = 3 + rawVal * maxLen * (0.85 + Math.sin(Date.now() * 0.003 + i * 0.5) * 0.15);
      ...
      ctx.shadowColor = active ? 'rgba(99, 102, 241, 0.5)' : 'rgba(0,0,0,0)';
      ...
      ctx.strokeStyle = active ? 'rgba(255, 255, 255, 0.15)' : 'rgba(255, 255, 255, 0.03)';
      ctx.lineWidth = 1;
      ctx.beginPath();
  ...
  return (
    <div className="relative w-full rounded-2xl flex flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-[var(--bg-surface)] to-[var(--bg-raised)] border border-[var(--border)] shadow-2xl p-8 pt-12 pb-6 transition-all duration-300">
      ...
      {active && (
        <div className="absolute left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-[#22d3ee]/35 to-transparent pointer-events-none animate-[holographic-scan_4s_linear_infinite]" />
      )}

      {/* Top UI Panel Overlay */}
      <div className="absolute top-4 left-6 right-6 flex items-center justify-between text-[10.5px] font-mono tracking-widest text-[var(--text-muted)] z-10">
      ...
      {/* ── Visualizer Sphere Container ── */}
      <div className="relative w-[230px] h-[230px] flex items-center justify-center mb-6 z-10">
        {/* Canvas fits perfectly inside this 230x230 container */}
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />
        
        {/* Glowing custom medical agent logo perfectly in center of visualizer orb */}
        <div className="z-10 flex items-center justify-center">
          <MedicalAgentLogo className="w-14 h-14" />
        </div>
      </div>

      {/* ── Status & Text Display - completely below the visualizer, zero overlap! ── */}
      <div className="text-center space-y-4 max-w-sm z-10">
        <div className="space-y-1.5">
          <h2 className={`text-[17px] font-extrabold tracking-wider transition-colors duration-300 ${active ? 'text-white' : 'text-[var(--text-secondary)]'}`}>
            {active ? 'Clinical Scribe Agent Active' : 'Scribe Standby'}
          </h2>
          <p className="text-[12.5px] text-[var(--text-muted)] font-medium leading-relaxed">
            {active 
              ? 'Listening and structuring the patient-doctor conversation in real-time.' 
              : 'Tap the microphone button below to initiate the clinical documentation session.'
            }
          </p>
        </div>
        ...
        {active && (
          <div className="inline-flex flex-col items-center gap-1 px-4 py-2 rounded-xl bg-black/40 border border-white/[0.05] backdrop-blur-md animate-in">
            <span className="text-[17px] font-mono font-bold text-indigo-400 tracking-wider tabular-nums">{fmt(elapsed)}</span>
            <div className="flex items-center gap-2.5 text-[10.5px] text-[var(--text-secondary)] font-mono">
